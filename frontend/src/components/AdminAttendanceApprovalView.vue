<template>
  <div class="admin-approval-container">
    <h2>勤怠・打刻修正承認パネル</h2>
    <p class="subtitle">
      全国のユーザーから提出された「打刻内容の修正申請」の一覧確認と、承認・差戻し処理が行えます。
    </p>
      ...
      >
        <div class="card-header">
          ...
          <span class="target-date"
            >対象日: {{ formatDate(req.recordDate) }}</span
          >
        </div>
          ...
          <div class="time-comparison-box">
            <div class="time-row origin-time">
              ...
              <span class="time-values"
                >Entry: <b>{{ req.entryTime }}</b> / Exit:
                <b>{{ req.exitTime }}</b></span
              >
            </div>
            ...
            <div class="time-row target-time">
              ...
              <span class="time-values"
                >Entry: <b class="highlight">{{ req.tmpEntryTime }}</b> / Exit:
                <b class="highlight">{{ req.tmpExitTime }}</b></span
              >
            </div>
          </div>

          <div class="reason-section">
            <strong>ユーザーからの申請理由・備考:</strong>
            <p class="reason-text">{{ req.reason }}</p>
          </div>

          <div
            v-if="activeRejectId === req.recordId"
            class="reject-comment-area"
          >
            <label
              >差戻し理由を入力してください
        ...
        <div class="card-actions">
          <template v-if="activeRejectId !== req.recordId">
            <button @click="handleApprove(req.recordId)" class="btn-approve">
              承認する
            </button>
            <button
              @click="showRejectInput(req.recordId)"
              class="btn-trigger-reject"
            >
              差戻す
            </button>
          </template>
          ...
          <template v-else>
            <button
              @click="handleReject(req.recordId)"
...
const handleApprove = async (recordId) => {
  ...
  try {
    ...
    const response = await apiClient.post("/admin/attendance/approve", {
      recordId,
    });
